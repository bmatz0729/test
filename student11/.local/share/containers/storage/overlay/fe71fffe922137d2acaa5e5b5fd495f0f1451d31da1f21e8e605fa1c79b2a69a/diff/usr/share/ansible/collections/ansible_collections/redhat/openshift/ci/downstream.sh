...
f_text_sub()
{
    ...
    OpenShift_sed_files="${_build_dir}/README.md ${_build_dir}/CHANGELOG.rst ${_build_dir}/changelogs/config.yaml ${_build_dir}/ci/downstream.sh ${_build_dir}/galaxy.yml"
    # shellcheck disable=SC2068
    for okd_file in ${OpenShift_sed_files[@]}; do sed -i.bak "s/OpenShift/OpenShift/g" "${okd_file}"; done
